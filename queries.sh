...
echo "$($PSQL "select max(greatest(winner_goals, opponent_goals)) as most_goals_scored from games;")"
...
echo "$($PSQL "select distinct name from teams right join games on teams.team_id = games.winner_id or teams.team_id = games.opponent_id where year = 2014 and round = 'Eighth-Final' order by name;")"
